...
#define emaill "^([a-z]+[,.][ ]?|[a-z]+['-]|[a-z]+[@])+$"
      QRegExp email(emaill);
        QRegExpValidator*valiEmail= new QRegExpValidator(email,this);
}
...
void MainWindow::on_pushButton_6_clicked()
{
  ...
  QString nom=ui->nom_a->text();
   QString prenom=ui->prenom_a->text();
    QString email=ui->email_a->setValidator(valiEmail);
